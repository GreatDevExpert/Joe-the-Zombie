...
@synthesize sticks = vSticks,_ropeSprites = ropeSprites;
...
-(void)createRope:(CGPoint)pointA pointB:(CGPoint)pointB distance:(float)distance {
    ...
    int segmentFactor;
    if (IS_IPAD) {segmentFactor = 24;}else{segmentFactor = 12;}
    ...
    if (IS_IPAD) {antiSagHack = 0.6f;}else{antiSagHack = 0.6f;}
	...
	if(spriteSheet!=nil) {
		for(int i=0;i<numPoints-1;i++) {
            ...
            float a;
            if (IS_IPAD) {a = -1;}else if (IS_IPHONE_5){a = 0;}else if (IS_IPHONE){a = 0;}
    ...
    //CCSprite *a = (CCSprite*)[ropeSprites objectAtIndex:15];
    //a.opacity = 0;
    
   // NSLog(@"ARRAY_______ :%f",[(VStick*)[vSticks objectAtIndex:1]getPointA].y);
}
