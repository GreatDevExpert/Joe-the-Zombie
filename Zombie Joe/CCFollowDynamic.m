...
+ (id) actionWithTarget:(CCNode *)fNode worldBoundary:(CGRect)rect smoothingFactor:(float)factor nodePlace:(float)nod_place_
{
    return [[[self alloc] initWithTarget:fNode worldBoundary:rect smoothingFactor:factor nodePlace:nod_place_] autorelease];
}
...
{
    return [self initWithTarget:fNode worldBoundary:rect smoothingFactor:0.0f nodePlace:0.5f];
}

- (id) initWithTarget:(CCNode *)fNode worldBoundary:(CGRect)rect smoothingFactor:(float)factor nodePlace:(float)nod_place_
{
    if( (self=[super init]) ) {
		followedNode_ = [fNode retain];
		boundarySet = TRUE;
		...
		halfScreenSize = ccpMult(fullScreenSize, nod_place_);
